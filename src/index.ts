...
import { addDailyGroupChallengeUpdates, generateScheduledNotifications, refreshAllChallenges } from "./store.js";
...
const { store, persist, heartRate, splits, performanceGoals } = await createProductionContext(config.databaseUrl, useDemoData);

createServer(store, config, persist, heartRate, splits, performanceGoals).listen(port, () => {
...
setInterval(async () => {
  try {
    ...
    generateScheduledNotifications(store);
    ...
    await persist({ kind: "notifications" });
